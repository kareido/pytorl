...
from pytorl.networks import Q_Network, init_network
...
def main():
    ...
    init_network(q_net, config.record.load_path, 'q_net.pth', obj_name='q_network')
    q_net = q_net.to(device)
    agent = DQN_Agent(device=device, q_net=q_net)
    agent.set_exploration(env.sample, lib.eps_greedy_func(config.greedy.end, config.greedy.end))
